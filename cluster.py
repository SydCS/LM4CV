...
def cluster(cfg):
    ...
    for i in range((len(attributes) // batch_size) + 1):
        ...
        attribute_embeddings += [
            ...
            for embedding in model.encode_text(clip_attributes_embeddings)
        ]
    ...
    if cfg["cluster_feature_method"] == "random":  # 1
    ...
    elif cfg["cluster_feature_method"] == "similarity":  # 2
        if cfg["model_type"] == "clip":
            model, preprocess = clip.load(cfg["model_size"])
        else:
            raise NotImplementedError
        ...
        if cfg["cluster_feature_method"] == "linear":  # 3 本方法
        ...
        elif cfg["cluster_feature_method"] == "kmeans":  # 4
            kmeans = KMeans(n_clusters=cfg["num_attributes"], random_state=0).fit(
                attribute_embeddings
            )
            centers = kmeans.cluster_centers_

        elif cfg["cluster_feature_method"] == "svd":  # 5
