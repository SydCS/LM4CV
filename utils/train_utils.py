...
def get_feature_dataloader(cfg):
    ...
    if cfg["model_type"] == "clip":
        ...
        model, _, preprocess = open_clip.create_model_and_transforms(
            cfg["model_size"], pretrained=cfg["openclip_pretrain"], device="cuda"
        )
    else:
        raise NotImplementedError
    ...
    train_loader = DataLoader(
        train_score_dataset, batch_size=cfg["batch_size"], shuffle=True
    )
    test_loader = DataLoader(
        test_score_dataset, batch_size=cfg["batch_size"], shuffle=False
    )
...
def get_score_dataloader(cfg, attribute_embeddings):
    ...
    test_loader = DataLoader(
        test_feature_dataset, batch_size=cfg["batch_size"], shuffle=False
    )
...
def train_model(
    ...
    model.cuda()
    optimizer = torch.optim.Adam([n] + list(model.parameters()), lr=cfg["lr"])
    ...
    for epoch in range(epochs):
        ...
        for idx, batch in enumerate(train_loader):
            ...
            if regularizer == "mahalanobis":
                ...
                weight = model[0].weight / model[0].weight.data.norm(
                    dim=-1, keepdim=True
                )
                loss += (
                    cfg["lambda"]
                    * torch.sum(
                        (weight - configs["mu"].unsqueeze(0).cuda()) ** 2, dim=-1
                    ).mean()
                )
        ...
        if epoch % 10 == 0:
            print(
                ...
                acc.item(),
                "Last Best accuracy:",
                last_best_acc.item() if last_best_acc is not None else 0,
            )
            sys.stdout.flush()
